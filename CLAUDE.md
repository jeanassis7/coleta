...
/api/admin/*                    → endpoints com service_role
  ...
  /cheques/rascunho (maço em andamento: GET/PUT/DELETE),
  /contas/pagamento (N contas × M meios; DELETE desfaz o acerto inteiro),
...
- `0078_rascunho_do_maco_de_cheques.sql` — `rascunho_lote_cheques`: **o maco de cheques em DUAS ETAPAS.** Fotografa no celular, confere no notebook com o papel na mesa. Guarda os DADOS lidos (inclusive as DIVERGENCIAS — valor extenso, ano assumido, origem do bom para; sem elas a 2a etapa recebe numero limpo onde havia duvida), **nunca a imagem**. Um maco por pessoa (PK no dono). Grava sozinho com folga de 1,2s — botao de salvar seria mais uma coisa pra esquecer. ⚠️ **Nao e dinheiro**: o maco so vira cheque pelo `/cheques/lote`, que segue validando tudo. ⚠️ A trava `jaCarregou` existe porque sem ela o estado vazio do primeiro render sobrescreveria o maco do celular assim que o notebook abrisse a tela
...
⚠️ **Uma origem pode ter MAIS DE UMA conta** (nota partida entre meios de pagamento — 0074). Consulta de conta por `origem_id` usa **`.limit(1)`, nunca `.maybeSingle()`**: com 2 linhas o `.maybeSingle()` devolve `data: null` e o erro que ninguém lê — a guarda para de guardar, calada. O pior caso já existia em `despesas/[id]`, onde isso fazia o código concluir "não existe conta" e **criar mais uma**. As 7 consultas do sistema foram blindadas em 15/09; consulta nova por origem nasce com `.limit(1)`.
